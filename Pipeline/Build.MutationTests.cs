				...
				string configText = $$"""
				                      {
				                      	"stryker-config": {
				                      		"project-info": {
				                      			"name": "github.com/Mockolate/Mockolate",
			...
			if (File.Exists(ArtifactsDirectory / "PR.txt"))
			{
				...
				if (int.TryParse(prNumber, out int prId))
				{
					...
					IReadOnlyList<IssueComment> comments =
						await gitHubClient.Issue.Comment.GetAllForIssue("Mockolate", "Mockolate", prId);
					long? commentId = null;
					...
					if (commentId == null)
					{
						...
						await gitHubClient.Issue.Comment.Create("Mockolate", "Mockolate", prId, body);
					}
					else
					{
						Log.Information($"Update comment:\n{body}");
						await gitHubClient.Issue.Comment.Update("Mockolate", "Mockolate", commentId.Value, body);
					}
				}
			}
		});
